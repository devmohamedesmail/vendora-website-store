...
export const selectWishlistItemById = (state: any, id: string) => 
  state.wishlist?.items?.find((item: WishlistItem) => item.id === id);
export const selectIsInWishlist = (state: any, id: string) => 
  state.wishlist?.items?.some((item: WishlistItem) => item.id === id) || false;
...
export const selectWishlistByVendor = (state: any, vendorName: string) => 
  state.wishlist?.items?.filter((item: WishlistItem) => item.vendor?.vendor_name === vendorName) || [];
